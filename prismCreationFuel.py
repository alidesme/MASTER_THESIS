...
TEMP_DIR = 'files'+os.sep+'prismSFuel'
# os.mkdir(TEMP_DIR)
...
class taxiEngine:
    ...
    def _initialize(self):
        ...
        print(
            f"formula distance_taxi_fuel = (max(xf-xt,xt-xf) + max(yf-yt,yt-yf));\n", file=self.prism_out)
    ...
    def _client(self, k):
        ...
        print(
            f"formula enough_fuel_c{k} = totalFuel >= ((distance_taxi_fuel) + (distanceX_c{k} + distanceY_c{k}));\n", file=self.prism_out)
    ...
    def _moduleArbiter(self):
        ...
        print('', file=self.prism_out)
        print('[pick_0] (token = 1) -> 1: (token\' = 2);', file=self.prism_out)
        print('[pick_1] (token = 2) -> 1: (token\' = 3);', file=self.prism_out)

        print('', file=self.prism_out)
        print('[client_0] (token = 3) -> 1: (token\' = 4);', file=self.prism_out)
        print('[client_1] (token = 4) -> 1: (token\' = 5);', file=self.prism_out)
    ...
    def _moduleTaxi(self):
        ...
        print("[North] (north) -> (fuelOK): (xt' = xt - 1) & (totalFuel' = totalFuel-1) + (1 - fuelOK) : (xt' = xt);", file=self.prism_out)
        print("[South] (south) -> (fuelOK): (xt'= xt + 1) & (totalFuel' = totalFuel-1) + (1 - fuelOK) : (xt' = xt);", file=self.prism_out)
        print("[East] (east) -> (fuelOK): (yt' = yt + 1) & (totalFuel' = totalFuel-1) + (1 - fuelOK) : (yt' = xt);", file=self.prism_out)
        print("[West] (west) -> (fuelOK): (yt' = yt - 1) & (totalFuel' = totalFuel-1) + (1 - fuelOK) : (yt' = xt);", file=self.prism_out)
    ...
    def _rewards(self):
        ...
        for k in range(self.number_of_clients):
            print(
                f'(reaching_c{k} & token = {k+3}): distanceX_c{k} + distanceY_c{k};', file=self.prism_out)
        print('endrewards\n', file=self.prism_out)
...
def getValue(prismFile, formula_str='R{"r"}max=? [F (reaching_c0 & token = 3)] '):
...
if __name__ == '__main__':
    ...
    # print(getValue("files/prismSFuel/420541_3.nm"))  # With fuel
