...
TEMP_DIR = 'files'+os.sep+'prismSWithout'
# os.mkdir(TEMP_DIR)
...
class taxiEngine:
    ...
    def _moduleArbiter(self):
        ...
        print('', file=self.prism_out)
        print('[pick_c0] (token = 1) -> 1: (token\' = 2);', file=self.prism_out)
        print('[pick_c1] (token = 2) -> 1: (token\' = 3);', file=self.prism_out)
    ...
    def _moduleClient(self):
        for k in range(self.number_of_clients):
            ...
            print(
                f'[client_{k}] (riding_c{k}) -> 1: (xc_c{k}\' = xc_c{k}) & (yc_c{k}\' =  yc_c{k}) ;', file=self.prism_out)
    ...
    def setRandomClientAttributes(self):
        ...
        random_destination_position = random.choice(
            temp)[0]  # TODO airport IS in destination too
...
# TODO What to satisfy ? #"Rmax=? [LRA]"& (((1-jam_int) * fuelOK)= 1) & jamCounter = 0 &
def getValue(prismFile, formula_str='R{"r"}max=? [F ((reaching_c1 & token=4) )]'):
...
if __name__ == '__main__':
    ...
    print(getValue(p))
    # print(getValue("files/prismSWithout/793803_3.nm"))   # Without any constraint
    pass
